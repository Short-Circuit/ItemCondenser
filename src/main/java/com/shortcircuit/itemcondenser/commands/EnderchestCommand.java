...
public class EnderchestCommand extends ShortCommand {
	...
	public String[] exec(CommandWrapper command)
		...
		if (command.getArgs().length > 0) {
			if (!PermissionComparator.hasWildcardPermission(command.getSender(), "itemcondenser.utility.enderchest.other")) {
				throw new NoPermissionException("itemcondenser.utility.enderchest.other");
			}
			player = plugin.getServer().getPlayer(command.getArg(0));
			if (player == null) {
